...
gwasPop <- c(popA, popB)
snpGeno <- as.data.frame(pullSnpGeno(gwasPop))
...
geno <- geno %>%
  mutate(chr = strtoi(sub("_.*", "", snp))) %>%
  mutate(pos = sub(".*_", "", snp))
colnames(geno)[-c(1:3)] = 1:gwasPop@nInd


pheno = data.frame(id = 1:gwasPop@nInd,
                    pheno  = gwasPop@pheno)
trait <- 2
