...
(plotTraitArchitecture(popA, "Fitness", twoTraitFitFunc) |
    plotTraitArchitecture(popB, "Fitness",twoTraitFitFunc))
...
n.subPopSize <- 100
n.nPops <- 2
source("CreateIndependentPops.R")
popA <- pops[[1]]
popB <- pops[[2]]
...
n.qtlPerChr <- 5
source("CreateFounderPop.R")
QtlGeno <- pullQtlGeno(foundingPop, trait=1)
...
Create NAM and GWAS Populations
```{r}
n.nPops <- 11
n.subPopSize <- 80
source("CreateIndependentPops.R")
NAM <- createNAM()
gwasPop <- c(pops[[1]], pops[[2]])
```
...
geno = data.frame(
  ...
  #pos = numeric(length(colnames(snpGeno))),
  t(snpGeno - 1)
)
geno <- geno %>%
...
genMap["snp"]
geno2 <- merge(x=geno, y=genMap, by="snp", all.x = TRUE)
geno2 <- geno[,c(1,2)]
genMap[["1_2"]]


geno["snp"]
geno["pos"] <- genMap[geno["snp"]]
geno["pos"] <- lapply(geno["pos"], function(x) genMap[geno$snp])
lapply
geno <- geno %>%
  mutate(pos = genMap$`1`[snp])
  #mutate(pos = strtoi(sub(".*_", "", snp)))
