...
for (s in 1:n.sims) {
  ...
  for(gen in 1:n.gens) {
    idx = (s-1)*n.gens + gen
    df$fitness[idx] <- oneTraitFitFunc(meanP(pop))
...
for(gen in 1:n.gens) {
  fit_df$fitness[gen] <- oneTraitFitFunc(meanP(pop))
...
SP <- SimParam$new(founders)
SP$addTraitA(mean=c(runif(1,-10,10),runif(1,-10,10)), var=c(1,1), nQtlPerChr=n.qtlPerChr)
...
for(gen in 1:n.gens) {
  ...
  pop <- selectCross(pop, trait=twoTraitFitFunc, nInd=n.popSize*0.5, nCrosses=n.popSize)
}
...
```{r}
adaptiveWalk <- function(nBurnInGens, nGens) {
  if (showPlot) {
  ...
  for (gen in 1:n.burnInGens) {
    ...
    pop <- selectCross(pop, trait=twoTraitFitFunc, nInd=nInd(pop)*burnInSelProp, nCrosses=nInd(pop))
  }
  
  # Create 2 sub populations
  # Figure out a better way to select randomly? or by traitA, traitB?
  popA <- selectInd(pop, use="rand", nInd=subn.popSize)
  popB <- selectInd(pop, use="rand", nInd=subn.popSize)
  ...
  for (gen in 1:nGens) {
    if (mean(twoTraitFitFunc(pheno(popA))) < n.margin) {
      popA <- selectCross(popA, trait=twoTraitFitFunc, nInd=nInd(popA)*selProp, nCrosses=nInd(popA))
      if (showPlot) {
    ...
    if (mean(twoTraitFitFunc(pheno(popB))) < n.margin) {
      popB <- selectCross(popB, trait=twoTraitFitFunc, nInd=nInd(popB)*selProp, nCrosses=nInd(popB))
      if (showPlot) {
...
popA_popB <- adaptiveWalk(nBurnInGens=n.burnInGens, nMaxSegGens=n.gens)
...
getSigQtl <- function(RIL, parentA, parentB) {
  ...
  operm.hk <- scanone(cross, pheno.col=phes, method="hk", n.perm=1000)
...
for (s in 1:n.sims) {
  print(paste0("Sim ", s))
  popA_popB <- adaptiveWalk(nBurnInGens=5, nMaxSegGens=100)
  popA <- popA_popB[1:subn.popSize]
  popB <- popA_popB[(subn.popSize+1):(subn.popSize*2)]
...
tot <- rbind(res.df[1:41,], res.df[51:90,])

ggplot(data=tot, aes(x=nSigQtl, fill=type)) +
