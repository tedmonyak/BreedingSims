...
res <- createRIL(pops[[1]], pops[[2]], save_dir)
# Parents A and B are the first two individuals in the result population
RIL <- res[-(1:2)]
parentA <- res[1]
parentB <- res[2]

getSigQtl(RIL, parentA, parentB, save_dir)
```
...
for (r in 1:n.popResets) {
  ...
  for (s in 1:n.sims) {
    ...
    res <- createRIL(pops[[1]], NA, save_dir)
    RIL <- res[-(1:2)]
    parentA <- res[1]
    parentB <- res[2]
...
g <- ggplot(data=res.df, aes(x=type, y=nSigQtl, fill=type)) +
      ...
      scale_fill_manual(name = "Parental Cross Type",
                        values = c("Inter" = "orange",
                                   "Intra" = "darkgreen"),
                        labels = c("Intra-population", "Inter-population")) +
...
ggplot(data=agg.df, aes(x=qtldiff)) +
