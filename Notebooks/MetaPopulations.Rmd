...
n.popResets <- 10
# Number of simulations per founder population
n.sims <- 10
# Result dataframe
...
for (r in 1:n.popResets) {
  ...
  for (s in 1:n.sims) {
    ...
    # By providing NA instead of a population, the createRIL function does an intra-population cross
    res <- createRIL(popA=pops[[1]], save_dir=save_dir, inter=FALSE)
    RIL <- res[-(1:2)]
    parentA <- res[1]
    parentB <- res[2]
