...
for (p in 1:length(subPopSizes)) {
  ...
  for(gen in 1:n.gens) {
    fit_df$fitness[gen] <- mean(twoTraitFitFunc(pheno(subPop)))
...
groupedEffSize.df <- effSize.df %>%
  mutate(smoothedOrder=ntile(orderFixed,20)) %>%
  group_by(smoothedOrder) %>%
...
g <- ggplot(groupedEffSize.df, aes(x=smoothedOrder, y=meanEffectSize)) +
  geom_point() +
  theme +
  labs(title="Figure 1", x="Order Fixed", y="Effect Size") +
