...
# Can be set as either 2 or 0, this is the allele to track frequency of
allele <- 2 
...
for(gen in 1:n.gens) {
  ...
  for (l in 1:length(qtl)) {
    ...
    freq.df[gen,id] <- (sum(locus==allele)/n.popSize) + ((sum(locus==1)/n.popSize)/2)
  }
...
#plotTraitArchitecture(founderPop) # REMOVE
```
...
selectSubPop <- function(x, idx) {
  as.numeric(lapply(randVec, idx, FUN=setequal))
}

# Select all of the "1" indexed individuals
popA <- selectInd(pop, trait=selectSubPop, selectTop=TRUE, nInd=n.subPopSize, idx=1)
# Select all of the "2" indexed individuals
popB <- selectInd(pop, trait=selectSubPop, selectTop=TRUE, nInd=n.subPopSize, idx=2)
